...
rp_addresses = ['192.168.0.18', '192.168.0.19', '192.168.0.17']
stragglers = ['192.168.0.17']
...
class Agg:
    ...
    def __init__(self):
        ...
        print("n: ", self._n)
        print("s: ", self._s)
        self._rand_data = build_random_matrix(self._length, np.array([1, 0.8, 0.7, 0.6]))
        print("rand_data: \n", self._rand_data[0:6, 0:6], "\n shape of rand_data: ", np.shape(self._rand_data))
        eig_vals, eig_vect = np.linalg.eig(self._rand_data)
    ...
    def receiving_loop(self):
        while True:
            ...
            self._item = server(laptop_address, 2, self._length)  # Update from 2 to (self._n - self._s)
    ...
    def action_loop(self):
        while True:
            if self._ready_to_process:
                ...
                if self._item.fn[0] == rp_addresses[0]:
                    w1 = 0
                    w2 = 2
                else:
                    w1 = 2
                ...
                error = np.linalg.norm(b_new - self._max_eigen)
                print("\nError: ", error)
                ...
                # Place the new vector in the que n-s times to be sent and update counters
                ...
                if self._count != self._iterations:
                    ...
                    print("final gradient vector:", b_new[0:10, :])
                    print("\nreceiving list: ", self._time_receive_list)
                    print("\nSending time list: ", self._time_send_list)
                    print("\n Processing list: ", self._action_time_list)
                    print("\n Error List: ", self._max_eigen)
                    ...
                    send_dict = {'Send': self._time_send_list}
                    receive_dict = {'Receive': self._time_receive_list}
                    action_dict = {'Action': self._action_time_list}
    ...
    # A function to process the time stamp data
