...
Nmax = comm.Get_size() - 1 # number of total machines
...
if rank == 0:
    print(option) # for testing

L = 10

Str_Tol = 2

K = L + Str_Tol # minimum number of machines + number of stragglers

# Matrix is q_coded*L by r
q_coded = 60
...
p_available = np.hstack((np.array([1 for n in range(K)]),np.array([0.5 for n in range(Nmax-K)]))) # speed of stable machines, elastic machines
...
comp_assignment = {'machines': [frozenset([int(n+1) for n in range(K)])],
                   'row_sets': [0,q_coded],
                   'w': []} # the vector for multiplication
...
for t in range(T):
    ...
    if rank == 0:  
        ...
        if option == 'HetAlgorithm':            
            ...
            if Nt == K:
                ...
                ind_unsort = np.argsort(ind_sort) # get back the orginal ordering
                k = Nmax
            ...
            alpha_sum = float(0) # tranking how much computation has been assigned
            ...
            for i in range(iteration):
                comp_assignment['row_sets'].append(int(alpha[i]*q_coded))    # this is what I need
        ...
        if option == 'HomCyclic':
            ...
            #######################
            comp_load = np.array([float(K) / float(Nt) for n in range(Nmax)]) # for tracking the speed 
    ...
    t1 = time.time() # track timing for speed estimation
    time.sleep(0.01)
    ...
    comp_coded['inv_time'] = float(1)/float(time.time() - t1) # for timing purposes
    ...
    if rank == 0:
        # Master node
        for i in range(len(comp_assignment['machines'])):
            # Master machine randomly selects L machines for decoding
            # Assignment can have >L machines from straggler tolerance
        ...
        C = C/np.linalg.norm(C) # Part of Power method design
        
    if rank == 0:
        # Master machine doing timing
        timeCount +=1
        timeStamps[timeCount] = time.time()
